...
export default function Login() {
    ...
    useEffect(() => {
        ...
        if (localStorage.auth_token) {
            fetch(`http://18.211.204.106/user/token-check`, loginHeader2)
                .then(response => {
    ...
    const handleLogin = event => {
        ...
        fetch(`http://18.211.204.106/user/signin`, loginHeader)
    ...
    const handleMailVerification = (event) => {
        ...
        fetch('http://18.211.204.106/user/forget-password', forgetPassHeader)
            .then(res => res.json())
            .then(data => {
                console.log(data);
                if (data.message) {
    ...
    const handlePassChange = (event) => {
        ...
        fetch('http://18.211.204.106/user/reset-password', resetPassHeader)
            .then(res => res.json())
            .then(data => {
                console.log(data);
                if (data.message){
